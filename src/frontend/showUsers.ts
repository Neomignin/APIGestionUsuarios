...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    result.data.forEach((user:any)=>{htmlUsers += `<tr><td>${user.name}</td><td>${user.first_surname}</td><td>${user.userName}</td><td>${user.email}</td><td><img class="update-button" id="update-${user.id}" src="../../media/icon/lapiz.png"></td><td><img class="delete-button" id="delete-${user.id}" src="../../media/icon/basura.png"></td></tr>`});
    htmlUsers += "</table>";
